...
2. Reddit Registry:
   ```bash
    ...
    ```
After obtaining the keys, load them into the ```.env ``` file and verify the configurations in ```config.py ``` as well as any files that import from it. # Mention which files import
